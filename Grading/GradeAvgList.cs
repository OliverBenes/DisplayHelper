...
namespace Grading
{
    class GradeAvgList : ICustomList
    {
        ...
        {
            GradeAvg[] newAvg = new GradeAvg[_gt.Length-1]; 
            for (int i = 0; i < IndexOf(g); i++)
            {
                newAvg[i] = _gt[i];
            }
            for(int i = IndexOf(g) + 1; i < _gt.Length; i++)
            {
                newAvg[i] = _gt[i];
            }
            _gt = newAvg;
        ...
        public bool Delete(int position)
        {
            ...
            {
                newAvg[i] = _gt[i];
            } //nahrání prvků až do position
            for (int i = position + 1;i<_gt.Length ; i++)
            {
                newAvg[i] = _gt[i];
            } //přeskočí prvek na position a nahraje zbylé
            _gt = newAvg;
            return true;
        }
